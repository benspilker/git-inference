#!/usr/bin/env python3
"""Stage-capable Playwright runner for Grok guest browser execution."""
...
from playwright_runner.grok_recovery import refresh_chat, start_new_chat_if_available
from playwright_runner.grok_stage_runner import run_stage_once, write_stage_outputs
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a named Playwright stage against Grok as guest.")
    ...
    parser.add_argument("--user-data-dir", default=".playwright-grok-profile")
    parser.add_argument("--omit-sections", default="")
    parser.add_argument("--start-new-chat", dest="start_new_chat", action="store_true", default=True)
    parser.add_argument("--no-start-new-chat", dest="start_new_chat", action="store_false")
    ...
    parser.add_argument("--network-log-file", default="grok_network.log")
    parser.add_argument("--error-screenshot", default="grok_playwright_error.png")
    parser.add_argument("--chunks", type=int, default=1)
    parser.add_argument("--chunk-mode", choices=["none", "map_reduce", "finalize_on_last_chunk", "legacy"], default="none")
    return parser.parse_args()


def assert_grok_domain(page) -> None:
    current_url = page.url or ""
    host = (urlparse(current_url).netloc or "").lower()
    allowed_hosts = ("grok.com", "x.com", "x.ai")
    if not any(h in host for h in allowed_hosts):
        raise RuntimeError(f"Unexpected browser destination: {current_url} (expected host containing one of: {', '.join(allowed_hosts)})")
