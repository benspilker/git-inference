#!/usr/bin/env python3
"""Stage-capable Playwright runner for Gemini browser execution."""
...
from playwright_runner.gemini_recovery import refresh_chat, start_new_chat_if_available
from playwright_runner.gemini_stage_runner import run_stage_once, write_stage_outputs
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a named Playwright stage against Gemini.")
    ...
    parser.add_argument("--user-data-dir", default=".playwright-gemini-profile")
    ...
    parser.add_argument("--network-log-file", default="gemini_network.log")
    parser.add_argument("--error-screenshot", default="gemini_playwright_error.png")
    parser.add_argument("--chunks", type=int, default=1)
    parser.add_argument("--chunk-mode", choices=["none", "map_reduce", "finalize_on_last_chunk", "legacy"], default="none")
    return parser.parse_args()


def assert_gemini_domain(page) -> None:
    current_url = page.url or ""
    host = (urlparse(current_url).netloc or "").lower()
    if "gemini.google.com" not in host:
        raise RuntimeError(f"Unexpected browser destination: {current_url} (expected host containing gemini.google.com)")
