...
def assistant_turns(page):
    selectors = [
        "div[id^='response-'].items-start div.response-content-markdown",
        "div[id^='response-'].items-start .response-content-markdown",
        "div[id^='response-'].items-start .message-bubble .response-content-markdown",
        "[data-testid='assistant-message']",
        "[data-author='assistant']",
        "[data-message-author='assistant']",
        "main [class*='response-content-markdown']",
    ...
    for selector in selectors:
        ...
        try:
            ...
            continue
    return page.locator("div[id^='response-'] .response-content-markdown")


def extract_response_text(assistant_messages, before_count: int, before_last_text: str) -> str:
    def _normalize(text: str) -> str:
        cleaned = (text or "").strip()
        cleaned = re.sub(r"^\s*grok\s+said:\s*\n+", "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"^\s*grok\s*:\s*\n+", "", cleaned, flags=re.IGNORECASE)
