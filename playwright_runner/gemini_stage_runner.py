...
from .gemini_recovery import click_retry_if_visible, refresh_chat
from .gemini_session import (
    assistant_turns,
    find_chat_composer,
    page_indicates_transient_error,
    send_prompt,
    stabilize_response,
...
def run_stage_once(
    ...
    def _single_attempt():
        if page_indicates_transient_error(page):
            raise RuntimeError("Gemini returned a transient service error page before prompt submission.")
        composer = find_chat_composer(page, timeout_ms=timeout_ms)
        if composer is None:
            if page_indicates_transient_error(page):
                raise RuntimeError("Gemini returned a transient service error page while waiting for composer.")
        ...
        if not response_text:
            if page_indicates_transient_error(page):
                raise RuntimeError("Gemini returned a transient service error page after prompt submission.")
