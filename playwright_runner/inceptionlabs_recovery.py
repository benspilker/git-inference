...
from .inceptionlabs_session import find_chat_composer
...
def _is_inceptionlabs_host(url: str) -> bool:
    host = (urlparse(url or "").netloc or "").lower()
    return "inceptionlabs.ai" in host


def ensure_inceptionlabs_surface(page, timeout_ms: int, home_url: str = "https://chat.inceptionlabs.ai/"):
    current_url = page.url or ""
    if _is_inceptionlabs_host(current_url):
        return page
    ...
    try:
        ...
        pass
    raise RuntimeError(
        f"Unexpected browser destination: {last_url} (expected host containing inceptionlabs.ai)"
    )


def start_new_chat_if_available(page, timeout_ms: int, home_url: str = "https://chat.inceptionlabs.ai/") -> bool:
    ...
    for selector in selectors:
        ...
        try:
            if candidate.count() > 0 and candidate.is_visible():
                ...
                current_url = page.url or ""
                if not _is_inceptionlabs_host(current_url):
                    try:
                        ensure_inceptionlabs_surface(page, timeout_ms=chat_timeout_ms, home_url=home_url)
                    except Exception:
                        continue
...
def refresh_chat(page, timeout_ms: int, home_url: str = "https://chat.inceptionlabs.ai/") -> None:
    current_url = page.url or ""
    if not _is_inceptionlabs_host(current_url):
        try:
            page.goto(home_url, wait_until="domcontentloaded", timeout=timeout_ms)
